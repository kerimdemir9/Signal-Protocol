...
def generate_Ks_sender(IK_Pub_b, Ek_Pri_sender, SPK_Pub_b, IK_Pri_a, OTK_Pub_b):
    T1 = IK_Pub_b * Ek_Pri_sender
    T2 = SPK_Pub_b * Ek_Pri_sender
    T3 = OTK_Pub_b * Ek_Pri_sender
    T4 = SPK_Pub_b * IK_Pri_a
    U = concatenate(T1.x, T1.y) + concatenate(T2.x, T2.y) + concatenate(T3.x, T3.y) + concatenate(T4.x,
                                                                                                  T4.y) + b'WhatsUpDoc'

    return int(SHA3_256.new(U).hexdigest(), 16)
...
if helper.SignVer(concatenate(SPK_Pub.x, SPK_Pub.y), h_sign_pre_key, s_sign_pre_key, curve, IK_Pub):
    ...
    cipher_text = aes.encrypt(bytes(message.encode()))

    H_MAC = HMAC.new(K_HMAC.to_bytes((K_HMAC.bit_length() + 7) // 8, byteorder="big"), msg=cipher_text,
                     digestmod=SHA256)

    MAC = ((int.from_bytes(H_MAC.digest(), byteorder="big") % n)
           .to_bytes(((int.from_bytes(H_MAC.digest(), byteorder="big") % n).bit_length() + 7) // 8, byteorder='big'))

    encrypted_message = int.from_bytes(nonce_to_byte + cipher_text + MAC, byteorder="big")